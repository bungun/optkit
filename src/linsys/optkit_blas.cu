...
{
	ok_status err = OPTKIT_SUCCESS;
	OK_CHECK_PTR(blas_handle);
	OK_CHECK_VECTOR(x);
	OK_CHECK_VECTOR(y);
	if (x->size != y->size)
		return OK_SCAN_ERR( OPTKIT_ERROR_DIMENSION_MISMATCH );

	err = OK_SCAN_CUBLAS( CUBLAS(axpy)(*(cublasHandle_t *) blas_handle,
...
ok_status blas_nrm2(void *blas_handle, const vector *x, ok_float *result)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(nrm2)(*(cublasHandle_t *) blas_handle,
...
ok_status blas_scal(void *blas_handle, const ok_float alpha, vector *x)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(scal)(*(cublasHandle_t *) blas_handle,
...
ok_status blas_asum(void *blas_handle, const vector *x, ok_float *result)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(asum)(*(cublasHandle_t *) blas_handle,
		(int) x->size, x->data, (int) x->stride, result) );
	cudaDeviceSynchronize();
	return err;
}
	...
	ok_float *result)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(dot)(*(cublasHandle_t *) blas_handle,
...
// void blas_dot_inplace(void *blas_handle, const vector *x, const vector *y,
// 	ok_float *deviceptr_result)
// {
// 	CUBLAS(dot)(*(cublasHandle_t *) blas_handle, (int) x->size, x->data,
	...
	vector *y)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(gemv)(*(cublasHandle_t *) blas_handle,
	...
	vector *x)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(trsv)(*(cublasHandle_t *) blas_handle,
	...
	const vector *vecA, const vector *x, const ok_float beta, vector *y)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(sbmv)(*(cublasHandle_t *) blas_handle,
	...
	const vector *vecA, const vector *x, const ok_float beta, vector *y)
{
	...
	err = OK_SCAN_ERR( blas_sbmv(blas_handle, CblasColMajor,
	...
	ok_float beta, matrix *C)
{
	...
	err = OK_SCAN_CUBLAS( CUBLAS(syrk)(*(cublasHandle_t *) blas_handle,
