...
void POGS(pogs_matrix_free)(pogs_matrix * M)
{
	PROJECTOR(free)(M->P);
	ok_free(M->P);
	matrix_free(M->A);
...
/* STUB */
ok_float POGS(estimate_norm)(void * linalg_handle, pogs_matrix * M)
{
	return kOne;
}

void POGS(normalize_DAE)(void * linalg_handle, pogs_matrix * M)
{
