 ...
 * ==========================
 */
static __global__ void __vector_set(ok_float * data, ok_float val, size_t stride,
	size_t size)
{
...
static void __vector_set_all(vector * v, ok_float x)
{
	uint grid_dim = calc_grid_dim(v->size);
	__vector_set<<<grid_dim, kBlockSize>>>(v->data, x, v->stride, v->size);
}

static __global__ void __strided_memcpy(ok_float * x, size_t stride_x,
 ...
 * ==============
 */
static inline int __vector_exists(vector * v)
{
	if (v == OK_NULL) {
...
/* row major setter */
static __global__ void __matrix_set_r(ok_float * data, ok_float x, size_t stride,
...
/* column major setter */
static __global__ void __matrix_set_c(ok_float * data, ok_float x, size_t stride,
	size_t size1, size_t size2)
{
	uint i, j;
	uint thread_id_row = blockIdx.x * blockDim.x + threadIdx.x;
...
static void __matrix_set_all(matrix * A, ok_float x)
{
...
static __device__ inline ok_float& __matrix_get_r(ok_float * A, uint i, uint j,
	uint stride)
{
	return A[i * stride + j];
}
...
static __device__ inline ok_float& __matrix_get_c(ok_float * A, uint i, uint j,
	uint stride)
{
	return A[i + j * stride];
}
	...
	size_t stride_minima, const enum CBLAS_ORDER ord)
{
	...
	minsub[row] = MATH(fmin)(minsub[row], get(Asub, row, col, kTileLD));

	compare = get(Asub, row, col, kTileLD);
	...
	size_t stride_minima, const enum CBLAS_ORDER ord)
{
	...
	minsub[col] = MATH(fmin)(minsub[col], get(Asub, row, col, kTileLD));

	compare = get(Asub, row, col, kTileLD);
	if (minsub[col] <= compare) {
		minsub[col] = compare;
		indminsub[col] = global_row;
	}
	__syncthreads();
...
static __global__ void __matrix_row_min(ok_float * minima, ok_float * A,
	size_t i, size_t j, size_t ld, size_t stride_minima, const enum CBLAS_ORDER ord)
{
	uint col, row, global_col, global_row;
	const uint kTileLD = kTileSize + 1u;
	__shared__ ok_float Asub[kTileLD * kTileSize];
...
static __global__ void __matrix_row_max(ok_float * maxima, ok_float * A,
	size_t i, size_t j, size_t ld, size_t stride_maxima, const enum CBLAS_ORDER ord)
{
	uint col, row, global_col, global_row;
	const uint kTileLD = kTileSize + 1u;
	__shared__ ok_float Asub[kTileLD * kTileSize];
	...
	maxsub[row] = MATH(fmax)(maxsub[row], get(Asub, row, col, kTileLD));
	__syncthreads();
	...
	size_t i, size_t j, size_t ld, size_t stride_maxima, const enum CBLAS_ORDER ord)
{
	...
	maxsub[col] = MATH(fmax)(maxsub[col], get(Asub, row, col, kTileLD));
	__syncthreads();
 ...
 * ==============
 */
static inline int __matrix_exists(matrix * A)
{
	if (A == OK_NULL) {
...
static __global__ void __block_chol(ok_float * A, uint iter, uint ld, const enum CBLAS_ORDER ord)
{
...
static __global__ void __block_trsv(ok_float * A, uint iter, uint n, uint ld, const enum CBLAS_ORDER ord)
{
	...
	matrix * A, const enum CBLAS_SIDE side)
{
	...
	cudaStream_t stm;
	uint num_row_tiles, num_col_tiles, i, j;
	size_t preserved_dim = (side == CblasLeft) ? A->size2 : A->size1;
	ok_float * minima;
	...
	ok_alloc_gpu(minima, preserved_dim * sizeof(*minima));
	__vector_set<<<grid_dim, kBlockSize>>>(minima, OK_FLOAT_MAX, 1,
		preserved_dim);
	...
	for (i = 0; i < num_row_tiles; ++i) {
		...
		for (j = 0; j < num_col_tiles; ++j) {
			...
			dim3 block_dim(block_dim_v, block_dim_h);
			reduce<<<1, block_dim, 0, stm>>>(indices, minima,
				A->data, i, j, (uint) A->ld, 1, 1, A->order);
			CUDA_CHECK_ERR;
		}
	}
	ok_free_gpu(minima);
