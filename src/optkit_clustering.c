...
cluster_aid *
cluster_aid_alloc(size_t m, size_t k){
    cluster_aid * h = (cluster_aid *) malloc( sizeof(cluster_aid) );
...
void 
cluster_aid_free(cluster_aid * h){
    vector_free(h->ck2);
    vector_free(h->ck2_);
    matrix_free(h->D);
    ok_free(h->ck2);
...
void
cluster_aid_sub(cluster_aid * hsub, cluster_aid * h, size_t m1, size_t k1){
...
void 
k_means(void * linalg_handle, matrix * A, matrix * C, 
