...
static ok_float __dist_lInf_A_minus_UC_i(const ok_float * A,
	...
	ok_float * A_blk, const int * strideBlk, const size_t * iterBlk,
	const * size_t blk, const size_t * i, const int * row_length)
	)
{
	CBLAS(copy)(*row_length, A + (*i) * (*iterA), (*strideA),
		A_blk->data + (*i - *blk) * (*iterBlk), (*strideBlk);
	CBLAS(axpy)(row_length, -kOne,
		C + u->indices[(*i) * u->stride] * (*iterC), (*strideC),
		A_blk->data + (*i - *blk) * (*iterBlk), (*strideBlk);
	return Ablk[CBLASI(amax)(row_length, A_blk + (*i - *blk) * (*iterBlk),
		(*strideBlk)) * (*strideBlk) + (*i - *blk) * (*iterBlk)];
}
...
{
	size_t i, blk, row_stride, row_strideA, row_strideC;;
	int idx_stride, strideA, strideC;
	matrix * A_blk;
	upsamplingvec * u;
	...
	idx_stride = (A_blk->order == CblasRowMajor) ? 1 : (int) A_blk->ld;
	...
	for (blk = 0; blk < A->size1; blk += kBlockSize)
		...
		for (i = blk; i < blk + kBlockSize && i < A->size1; ++i)
			if (a2c->indices[i] == u->indices[i]) {
				...
				&row_stride, &blk, &i, (const int) A->size2) {
