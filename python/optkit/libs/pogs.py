...
def attach_pogs_adaptrho_ctypes(lib, single_precision=False):
    ...
    # TODO: keep spectral rho?
    class SpectralRhoParameters(ct.Structure):
        _fields_ = [('dH', lib.vector_p),
                    ('dG', lib.vector_p),
                    ('dlambda', lib.vector_p),
                    ('dlambda12', lib.vector_p)]
...
def _attach_pogs_adaptrho_ccalls(lib):
    ...
    OptkitLibs.attach_default_restype(
            lib.pogs_adaptive_rho_initialize,
            lib.pogs_adapt_rho)

    # TODO: keep spectral rho?
    lib.pogs_spectral_rho_initialize.argtypes = [
            lib.spectral_rho_params_p, ct.c_size_t]
    lib.pogs_spectral_rho_free.argtypes = [lib.spectral_rho_params_p]
    lib.pogs_spectral_update_start.argtypes = [
            lib.spectral_rho_params_p, lib.pogs_variables_p, lib.ok_float]
    lib.pogs_spectral_update_end.argtypes = [
            ct.c_void_p, lib.spectral_rho_params_p, lib.pogs_variables_p,
            lib.ok_float]
    lib.pogs_spectral_estimate_tangent.argtypes = [
            ct.c_void_p, lib.vector_p, lib.vector_p, lib.ok_float_p,
            lib.ok_float_p]
    lib.pogs_spectral_adapt_rho.argtypes = [
            ct.c_void_p, lib.pogs_variables_p, lib.ok_float_p,
            lib.spectral_rho_params_p, lib.pogs_settings_p, ct.c_uint]
