...
class ClusterLibsTestCase(OptkitCTestCase):
	...
	def test_blockwise_kmeans(self):
		...
		for (gpu, single_precision) in CONDITIONS:
			...
			blksA = np.zeros(BLOCKS).astype(c_size_t)
			blksA_ptr = blksA.ctypes.data_as(dlib.c_size_t_p)
			blksA += m / BLOCKS
			blksA[-1] += m % BLOCKS

			blksC = np.zeros(BLOCKS).astype(c_size_t)
			blksC_ptr = blksC.ctypes.data_as(dlib.c_size_t_p)
			blksC += k / BLOCKS
			blksC[-1] += k % BLOCKS
			...
			for b in xrange(BLOCKS):
				...
				a2c_rand = (offset_C + blocksize_C *
							np.random.rand(blocksize_A)).astype(c_size_t)

				a2c_py[offset_A : offset_A + blocksize_A] *= 0
				a2c_py[offset_A : offset_A + blocksize_A] += a2c_rand

				offset_A += blocksize_A
				offset_C += blocksize_C

			dlib.indvector_memcpy_va(a2c.vec, a2c_ptr, 1)

			change_tols = np.array(BLOCKS * [CHANGE_TOL]).astype(c_size_t)
			change_tols_ptr = change_tols.ctypes.data_as(dlib.c_size_t_p)

			err = lib.blockwise_kmeans(A, C, a2c, counts, BLOCKS, blksA_ptr,
									   blksC_ptr, MAXITER, MAXDIST,
									   change_tols_ptr, VERBOSE)
