...
def objectives_are_scaled(lib, solver, test_params):
    ...
    RTOL, ATOLM, ATOLN, _ = okctest.standard_tolerances(lib, m, n)
    ...
    assert okctest.vec_equal( f_d0, local_vars.d * f_d1, ATOLM, RTOL )
    assert okctest.vec_equal( f_e0, local_vars.d * f_e1, ATOLM, RTOL )
    assert okctest.vec_equal( g_a0 * local_vars.e, g_a1, ATOLN, RTOL )
    assert okctest.vec_equal( g_d0 * local_vars.e, g_d1, ATOLN, RTOL )
    assert okctest.vec_equal( g_e0 * local_vars.e, g_e1, ATOLN, RTOL )

    # unchanged vars
    assert okctest.vec_equal( f_h0, f_h1, ATOLM, RTOL )
    assert okctest.vec_equal( f_b0, f_b1, ATOLM, RTOL )
    assert okctest.vec_equal( f_c0, f_c1, ATOLM, RTOL )
    assert okctest.vec_equal( g_h0, g_h1, ATOLN, RTOL )
    assert okctest.vec_equal( g_b0, g_b1, ATOLN, RTOL )
    assert okctest.vec_equal( g_c0, g_c1, ATOLN, RTOL )
...
def matrix_is_equilibrated(lib, solver_work, A_orig):
    ...
    RTOL, ATOLM, _, _ = okctest.standard_tolerances(lib, m, n)
    ...
    with x, y:
        if lib.py_pogs_impl == 'dense':
            ...
            assert okctest.noerr( lib.blas_gemv(hdl, tr, 1, A, x.c, 0, y.c) )
        elif lib.py_pogs_impl == 'abstract':
            opA = solver_work.contents.A
            assert okctest.noerr( opA.contents.apply(opA.contents.data, x.c, y.c) )
        else:
            raise ValueError('UNKNOWN POGS IMPLEMENTATION')

        y.sync_to_py()
        DAEX = d_local * A_orig.dot(e_local * x.py)
        return okctest.vec_equal( y.py, DAEX, ATOLM, RTOL )
...
def adjoint_multiply_works(lib, work, A_equil):
    ...
    alpha = np.random.random()
    beta = np.random.random()
    RTOL, _, ATOLN, _ = okctest.standard_tolerances(lib, m, n)
    ...
    with x, y:
        ...
        x.sync_to_py()
        return okctest.vec_equal( x.py, ATYPX, ATOLN, RTOL )
...
def graph_projector_works(lib, work, A_equil):
    ...
    with x_in, y_in, x_out, y_out:
        ...
        assert okctest.noerr(getattr(
                lib, project)(work, x_in.c, y_in.c, x_out.c, y_out.c, RTOL))

        x_out.sync_to_py()
        y_out.sync_to_py()
        AX = A_equil.dot(x_out.py)
        return okctest.vec_equal( y_out.py, AX, ATOLM, RTOL)
...
def proximal_update_is_correct(lib, solver, test_params):
    ...
    assert okctest.vec_equal( local_vars.x12, x_out, ATOLN, RTOL )
    assert okctest.vec_equal( local_vars.y12, y_out, ATOLM, RTOL )
    return True
...
def convergence_test_is_consistent(lib, solver, A_equil, test_params):
    ...
    assert okctest.scalar_equal( objectives.gap, obj_gap_py, RTOL )
    assert okctest.scalar_equal( tolerances.primal, tol_primal, RTOL )
    assert okctest.scalar_equal( tolerances.dual, tol_dual, RTOL )
    ...
    assert okctest.scalar_equal( residuals.primal, res_primal, RTOL )
    assert okctest.scalar_equal( residuals.dual, res_dual, RTOL )
    assert okctest.scalar_equal( residuals.gap, abs(obj_gap_py), RTOL )
...
def inputs_are_scaled(lib, A_equil, x0, nu0, local_vars, rho):
    ...
    assert okctest.vec_equal( x0, x_scal, ATOLN, RTOL )
    assert okctest.vec_equal( nu0, nu_scal, ATOLM, RTOL )
    assert okctest.vec_equal( y0_unscal, local_vars.y, ATOLM, RTOL )
    assert okctest.vec_equal( mu0_unscal, local_vars.xt, ATOLN, RTOL )
    return True
